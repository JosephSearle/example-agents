...
# ground-truth `expected_facts` to judge against. `build_production_scorers` derives the
# `gateway:/<route>` judge model `Scorer.start()` requires (unlike `_JUDGE_MODEL_URI` above's
# `openai:/<route>` form, which `mlflow.genai.evaluate()`'s judges use instead via the
# OPENAI_API_BASE env var trick in tests/evals/test_quality.py — confirmed against a live
# mlflow-server: `openai:/...` fails `Scorer.start()` with "does not use a gateway model").
# `concise_answer`'s guideline text is loaded from
# packages/mlflow-server/judges/react-agent-concise_answer.txt, the same source that eval suite
# loads it from — single source of truth, see agents_common.judges.
PRODUCTION_SCORERS: list[tuple[Any, float]] = build_production_scorers(
    GATEWAY_ROUTE, [("concise_answer", "react-agent-concise_answer")]
)
