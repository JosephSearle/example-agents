"""CLI entrypoint: `uv run self-rag-agent "<question>"`."""
...
def main() -> None:
    """Retrieve, grade, generate, and self-grade an answer, correcting via regenerate/retry as needed."""
    if len(sys.argv) < _MIN_ARGC:
        print('Usage: self-rag-agent "<question>"', file=sys.stderr)
        raise SystemExit(1)
