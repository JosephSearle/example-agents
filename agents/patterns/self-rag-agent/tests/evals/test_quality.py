...
@scorer
def correct_self_grading(outputs: dict[str, object], expectations: dict[str, object]) -> bool:
    """Exact-match check on `grade_generation`: for a well-grounded, useful answer, did the
    hallucination/usefulness self-graders correctly say so, rather than false-flagging it?
    """
    return (
        outputs["grounded"] == expectations["expected_grounded"]
        and outputs["useful"] == expectations["expected_useful"]
    )
...
def test_self_rag_agent_eval_suite() -> None:
    ...
    assert results.metrics["correct_self_grading/mean"] >= 0.7


@pytest.mark.regression
def test_self_rag_agent_regression() -> None:
    ...
    with mlflow.start_run(run_name="self-rag-agent-regression"):
        results = mlflow.genai.evaluate(
            data=records,
            predict_fn=_predict_fn,
            scorers=[correct_self_grading],
        )

    assert results.metrics["correct_self_grading/mean"] >= 0.95
