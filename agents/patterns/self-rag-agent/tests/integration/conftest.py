...
class _FakeGrader:
    """Always scores "yes" — integration tests exercise real Milvus retrieval, not the
    self-correction loops (those are covered hermetically in tests/unit)."""

    def invoke(self, _prompt: str) -> _FakeScore:
        return _FakeScore("yes")


class _FakeChatModel:
    """Always returns a canned response for generation; every grader always passes. Stands in
    for the real gateway chat model so checkpointing tests exercise real Postgres + real Milvus
    retrieval without depending on network access to a live chat model."""
...
def fake_chat_model(monkeypatch: pytest.MonkeyPatch) -> _FakeChatModel:
    """Patches `self_rag_agent.graph.get_chat_model` so `build_rag_graph()` never reaches the
    network — retrieval still hits the real, seeded Milvus collection."""
    model = _FakeChatModel()
    monkeypatch.setattr("self_rag_agent.graph.get_chat_model", lambda _route: model)
    return model
