"""MLflow GenAI eval suite for the Corrective RAG (CRAG) workflow.
...
`corrective_rag_agent.EXPERIMENT_NAME` experiment.

The dataset lives in MLflow's dataset registry, seeded from
`packages/mlflow-server/datasets/corrective-rag-agent.jsonl` via `make provision-datasets`.
"""
...
def test_corrective_rag_agent_eval_suite() -> None:
    ...
    with mlflow.start_run(run_name="corrective-rag-agent-eval"):
