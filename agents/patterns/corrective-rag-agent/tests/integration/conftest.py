...
class _FakeGrader:
    """Grades every document "yes" — integration tests exercise real Milvus retrieval, not the
    corrective-retry loop (that's covered hermetically in tests/unit)."""

    def invoke(self, _prompt: str) -> _FakeGrade:
        return _FakeGrade("yes")


class _FakeChatModel:
    """Always returns a canned response for generation; grading always passes via `_FakeGrader`.
    Stands in for the real gateway chat model so checkpointing tests exercise real Postgres +
    real Milvus retrieval without depending on network access to a live chat model."""
...
def fake_chat_model(monkeypatch: pytest.MonkeyPatch) -> _FakeChatModel:
    """Patches `corrective_rag_agent.graph.get_chat_model` so `build_rag_graph()` never reaches
    the network for grading/generation — retrieval still hits the real, seeded Milvus collection."""
    model = _FakeChatModel()
    monkeypatch.setattr("corrective_rag_agent.graph.get_chat_model", lambda _route: model)
    return model
