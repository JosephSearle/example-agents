"""CLI entrypoint: `uv run basic-rag-agent "<question>"`."""
...
from basic_rag_agent.graph import (
    EXPERIMENT_NAME,
    build_rag_graph,
    invoke_config,
...
def main() -> None:
    """Retrieve context for a question from Milvus, generate a grounded answer, and print it."""
    if len(sys.argv) < _MIN_ARGC:
        print('Usage: basic-rag-agent "<question>"', file=sys.stderr)
        raise SystemExit(1)
